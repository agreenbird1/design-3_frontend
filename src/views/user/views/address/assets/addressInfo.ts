import type { IAddressT } from "../types";
...
export const options: IAddressT[] = [];
provice.forEach((p) => {
  const children: IAddressT[] = [];
  p.city.forEach((c) => {
    const cchildren: IAddressT[] = [];
    c.districtAndCounty.forEach((d) => {
      cchildren.push({
        value: d,
        label: d,
      });
  ...
  const label = p.label;
  if (p.city.length === 1) {
    p.label = p.label.slice(0, p.label.length - 1);
  }
  options.push({
    value: p.label,
    label: label,
